#include "WRRFLCScheduler.h"

#include "src/Queue.h"

Define_Module(WRRFLCScheduler);

WRRFLCScheduler::WRRFLCScheduler() {
...
void WRRFLCScheduler::parseWeights(const char *weightsAsString) {
    sscanf(weightsAsString, "%d %d %d %d", (userWeights), (userWeights + 1),
            (userWeights + 2), (userWeights + 3));
}

void WRRFLCScheduler::initialize() {
...
void WRRFLCScheduler::handleMessage(cMessage *msg) {
...
void WRRFLCScheduler::sortTimes(simtime_t times[4]) {
    bool sw;
    int i;
    simtime_t aux;
...
int WRRFLCScheduler::getQueueLength(const char *queueName) {
    int queueLength;
...
int WRRFLCScheduler::getWeight(int index)
{
    return userWeights[index];
}

void WRRFLCScheduler::setWeight(int newWeight, int index)
{
    userWeights[index] = newWeight;
}
