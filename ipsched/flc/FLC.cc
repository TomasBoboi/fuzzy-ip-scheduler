...
void Matrix::min_matrix(Matrix *m2, Matrix *result)
{
    ...
    {
        cout << "ERROR!!! matrices have different dimensions\n";
    }
...
void Matrix::max_column(Vector *v)
{
    ...
    if (this->_m != v->get_m())
    {
        cout << "ERROR!!!dimensions not matching\n";
    }
...
{
    public:
        Term(int a, int b, int c, int d, string name) : _a(a), _b(b), _c(c), _d(d), _name(name) {}
...
int Term::getTermRep(int m, int n, Matrix *mat)
{
    ...
    if (_a < 0 || _a > n - 1 ||
        _b < 0 || _b > n - 1 ||
        _c < 0 || _c > n - 1 ||
        _d < 0 || _d > n - 1) { return -1; }

    if (_a > _b || _b > _c || _c > _d) { return -1; }
